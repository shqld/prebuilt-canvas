...
function loadBinding () {
  ...
  if (packageName) {
    try {
      ...
      if (process.platform === 'win32') {
        ...
        process.env.LD_LIBRARY_PATH = pkgDir + (process.env.LD_LIBRARY_PATH ? path.delimiter + process.env.LD_LIBRARY_PATH : '')
      }
      ...
      try {
        ...
        if (process.env.DEBUG_CANVAS) console.error('ABI binary load failed:', e.message)
      }

      // Fall back to generic canvas.node (v3/N-API builds)
      return require(packageName)
    } catch (e) {
      if (process.env.DEBUG_CANVAS) console.error('Platform package load failed:', e.message)
  ...
  try {
    ...
    if (process.platform === 'win32') {
      ...
      process.env.LD_LIBRARY_PATH = localPath + (process.env.LD_LIBRARY_PATH ? path.delimiter + process.env.LD_LIBRARY_PATH : '')
    }
    // Try ABI-specific first, then generic
    try {
      return require(path.join(localPath, `canvas-node-v${abi}.node`))
    } catch (e) {
      if (process.env.DEBUG_CANVAS) console.error('Local ABI binary load failed:', e.message)
      return require(path.join(localPath, 'canvas.node'))
    }
  } catch (e) {
    if (process.env.DEBUG_CANVAS) console.error('Local fallback failed:', e.message)
